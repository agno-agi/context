...
- **Pattern** — deterministic function tools registered straight on the agent (like the queue tools in [`agents/inbox.py`](../../../agents/inbox.py)) or a context provider — a sub-agent behind `query_<id>` / `update_<id>` tools, built by a `_create_<id>_provider()` factory in [`agents/sources.py`](../../../agents/sources.py).
- **Existing levers** — `learning=` (the `LearningMachine` and its store modes), `num_history_runs`, `knowledge=`, model id.
...
What to capture per branch (stacking multiple changes in one session? run the `agno-docs` lookups in parallel):
...
- **Add a capability**:
  - *Knowledge base* — build an agno `Knowledge` with a `PgVector` vector db (`db_url` from `db.url`), pass via `knowledge=` on the Agent. Document load step (`.add_content_async()`) goes wherever ingestion lives.
  - *Memory / learning* — the `learning=LearningMachine(...)` on the Agent constructor (per-store configs and `LearningMode`s — note the shared-scope stores `entity_memory` / `learned_knowledge` / `decision_log` inject context for *every* caller, so they must not be enabled on this agent; see `docs/SECURITY.md`), plus `add_history_to_context`, `num_history_runs`. Read agno's learning docs to pick the right store and mode.
  - *Source / context provider* — mirror a `_create_<id>_provider()` factory in [`agents/sources.py`](../../../agents/sources.py) and add it to `create_context_providers()` (env-gated providers go through the `try/except` loop). The main agent sees `query_<id>` (and `update_<id>` if writable); the sub-agent does the work.
...
- [`agents/<slug>.py`](../../../agents/) — tools, model, memory flags, knowledge. (For `context`, prompt text lives in [`agents/instructions.py`](../../../agents/instructions.py) and providers in [`agents/sources.py`](../../../agents/sources.py).)
...
```bash
docker exec context-api grep -c "<unique substring from your edit>" /app/agents/<slug>.py
```
...
- **Recommended next step** — run the `improve-agent` skill to autonomously verify the agent still does what its `INSTRUCTIONS` say it does. The change you just made widened the agent's surface area; the autonomous loop catches anything that regressed.
...
Target: `context`. The user wants @context to also read their Google Drive.

**Step 2** — user picks "Add a capability" (a new source / context provider).

**Step 3** — search the agno-docs MCP for the gdrive context provider. Capture the import path, constructor args, required env vars / auth, and any pip deps.

**Step 4** — propose: *"Add an env-gated `gdrive` source: a `_create_gdrive_provider()` factory in `agents/sources.py`, wired through the `try/except` loop in `create_context_providers()` so a bad credential can't take the registry down. Document the env vars in `example.env`. Add a quick prompt that exercises it."* User says yes.

Edit [`agents/sources.py`](../../../agents/sources.py) (factory + registry — the provider exposes `query_gdrive` to the agent automatically; no change to `context.py`). Add the env vars to [`example.env`](../../../example.env). Add a quick prompt to `app/config.yaml`:
...
**Step 5** — pip deps changed? `./scripts/generate_requirements.sh && docker compose up -d --build`; otherwise hot-reload picks up `agents/` edits in ~1s. Poll `/health`.

**Step 6** — cURL the `context` agent with the quick prompt. Logs show `Running: query_gdrive(`. Response is grounded in the doc.

**Step 7** — user says "no, that's it."

**Step 8** — diff summary, commit `feat(context): add gdrive source`, recommend `improve-agent` to harden the broader behavior.
